import { FlightFormValues } from "@/types/logs";

const DRAFT_COOKIE_NAME = "nplb_flight_form_draft";
const DRAFT_COOKIE_MAX_AGE = 60 * 60 * 24 * 7; // 7 days

export function getDraftFromCookie(): FlightFormValues | null {
  ...
  try {
    ...
    console.error("Failed to parse flight draft cookie:", error);
    return null;
  }
}

export function saveDraftToCookie(data: FlightFormValues) {
  ...
  try {
    ...
    console.error("Failed to save flight draft cookie:", error);
  }
}
