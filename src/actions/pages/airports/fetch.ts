...
/**
 * Resolve coordinates + country for a specific list of ICAO codes.
 * Used by the dashboard airports map — only fetches what it needs.
 */
export async function getAirportCoordinates(
  codes: string[],
): Promise<
  Result<
    Pick<Airport, "icao" | "lat" | "lon" | "countryCode" | "countryName">[]
  >
> {
  "use cache";
  cacheLife("hours");
  ...
  const resolved = result.data
    .filter((a) => codeSet.has(a.icao.toUpperCase()))
    .map(({ icao, lat, lon, countryCode, countryName }) => ({
      icao,
      lat,
      lon,
      countryCode,
